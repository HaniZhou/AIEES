from app.model.schema.course import ResourceType
from typing import List, Any, Optional
from sqlmodel import SQLModel, Field, Relationship
from sqlalchemy import Column, JSON, TEXT, SMALLINT, UniqueConstraint, Enum, DateTime
import uuid
from sqlalchemy import Column, JSON, TEXT, SMALLINT, UniqueConstraint, Enum as SAEnum
...
class Teacher(UserSQLBase, table=True):
    ...
    organization: Optional[Organization] = Relationship(back_populates="teachers", passive_deletes="all",
                                                        sa_relationship_kwargs={"lazy": "raise"})
...
class Course(SQLModel, table=True):
    ...
    teaching_analysis: Optional[str] = Field(default=None, sa_column=Column(TEXT), description="GAI分析教学分析")
...
class Section(SQLModel, table=True):
    ...
    section_type: ResourceType = Field(sa_column=Column(Enum(ResourceType)))
...
class Task(SQLModel, table=True):
    ...
    quiz: Optional[list[dict[str, Any]]] = Field(default=None, sa_column=Column(JSON))
    answer: Optional[list[dict[str, Any]]] = Field(default=None, sa_column=Column(JSON))
...
class TaskCompletion(SQLModel, table=True):
    ...
    task_analysis: Optional[str] = Field(default="AI正在分析中", sa_column=Column(TEXT),
...
class AnalysisTaskCompletion(SQLModel, table=True):
    ...
    student_id: str = Field(foreign_key="student.id", ondelete="CASCADE")
    messages: Optional[dict[str, Any]] = Field(default=None, sa_column=Column(JSON), description="存储对话massages")
...
class StudentDailyStudyTimeInCourse(SQLModel, table=True):
    ...
    course_id: uuid.UUID = Field(index=True, foreign_key="course.course_id", ondelete="CASCADE")
    study_data: Optional[list[int | None]] = Field(default=None, sa_column=Column(JSON),
