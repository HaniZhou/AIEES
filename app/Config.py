...
class Prompt:
   ...
   """,
    ...
    - 优秀：主动且准确地运用，能自我纠错或迁移（例：主动建构逻辑链并验证边缘情况）。
    - 良好：有意识主动尝试，基本方向正确但存在细微漏洞（例：尝试用证据支撑主张，但关联度稍弱）。
    - 待发：表现出初步意识并开始尝试，但方向存在明显偏差或关联断裂（例：尝试寻找证据但匹配错误，或需轻微提示才能推进）。
    - 薄弱：高度依赖外部提示，仅有零星表现无法独立完成（例：需AI追问才想起寻找证据，或只能完成单步推理）。
    - 缺失：未展现该维度意识或完全反向操作（例：直接索要答案，无任何推理过程）。
    </SCORING_RUBRIC>
    <ANALYSIS_FRAMEWORK>
    ...
    **【素养四维体检表】**
    | 评估维度 | 维度评分 | 诊断标签 | 行为证据 (极简概括) |
    | :--- | :---: | :--- | :--- |
    | 知识解释 | [缺失/薄弱/待发/良好/优秀] | [概念清晰/迷思前置/语意模糊] | *例：准确使用XX概念* |
    | 逻辑推演 | [缺失/薄弱/待发/良好/优秀] | [假设驱动/机械验证/被动接受] | *例：仅索要步骤* |
    | 批判反思 | [缺失/薄弱/待发/良好/优秀] | [质疑辨伪/逻辑盲区/全盘接受] | *例：未察觉矛盾条件* |
    | AI协同 | [缺失/薄弱/待发/良好/优秀] | [苏格拉底式/简单指令型/无策略] | *例：直接索要答案* |
    
    ---
    
    #### 1. 知识调用与解释
    > **行为证据**：『在此严格逐字截取学生原话』
    - **认知切片**：此处该生将XX与XX混淆，导致推理链断裂。
    
    #### 2. 逻辑推演与论证
    > **行为证据**：『在此严格逐字截取学生原话』
    - **认知切片**：该生在此处仅索要结论，未尝试建立证据与主张的关联。
    
    #### 3. 批判与反思
    > **行为证据**：『在此严格逐字截取学生原话』
    - **认知切片**：面对AI给出的矛盾条件，该生未表现出质疑，直接全盘接受。
    
    #### 4. GAI协同素养
    > **行为证据**：『在此严格逐字截取学生原话』
    - **认知切片**：该生使用简单指令索取答案，缺乏对提示词结构的主动设计。
    
    ---
    
    **【人机协同模式诊断】**
    - **模式判定**：[检索工具型 / 苏格拉底型 / 简单指令型]
    - **策略建议**：该生在交互中倾向于[具体行为]，建议从[具体策略]丰富其与AI的协作策略...
    
    **【学生整体认知综诉】**
    - **认知模式总结**：综合来看，该生目前处于从[具体状态]向[具体状态]过渡的阶段，其核心认知特征为[概括核心优点与卡点]...
    - **高维度教学方向**：建议下一步教学重点放在[教学建议]，以促进其向更高素养等级发展。
    </OUTPUT_STRUCTURE>
    <RED_LINES>
    - 零幻觉：所有诊断必须100%溯源至 <INPUT_DATA>，严禁脱离文本泛泛而谈！
    - 拒绝对人定性：禁用“差生/聪明/盲从/依赖”等标签，只描述“该生在XX具体认知环节存在XX模式/障碍”！
    - [具体状态]和[教学建议]等标签是输出结构中的占位符，必须替换为具体内容，不要保留占位符！严禁输出任何未替换的占位符标签！
    </RED_LINES>
    """
